# Сайт odrodok.ru

Статический сайт, без сборщиков и зависимостей. Открывается прямо из файла.

## Структура

    site/
      index.html            → odrodok.ru        (заглушка со знаком)
      pervoprohodcy.html    → odrodok.ru/pervoprohodcy
      vozduh.html           → odrodok.ru/vozduh
      favicon.ico
      site.webmanifest
      .htaccess             адреса без .html, кеш, сжатие
      assets/
        tokens.css          дизайн-токены: цвета, отступы, тени, типографика
        tokens.json         те же токены в формате W3C Design Tokens (для Figma)
        fonts/              ocra-becker.woff2 (основной), .otf (запасной), basis-400
        icons/              иконки под браузер и iOS
        mark.webp           знак в шапке и подвале
        pervoprohodcy/      обложка, кадры, og
        vozduh/             кадры, og
      src/                  исходные тексты замыслов
      src/_archive/         черновики, которые пока не в работе
      tools/nbsp.py         типограф: убирает висячие предлоги
      dist/                 архивы сборок для заливки на хостинг

## Как добавить страницу

1. Скопировать `vozduh.html` (в нём нет плеера) или `pervoprohodcy.html` (с плеером).
2. Поменять `title`, `description`, og-теги, `h1`, текст, кадры.
3. Кадры: класть в `assets/<проект>/`, конвертировать в webp.
4. Прогнать типограф: `python3 tools/nbsp.py новая-страница.html`

## Хостинг: как попасть в панель

Прямая ссылка на ISPmanager (`server68.hosting.reg.ru:1500`) НЕ пускает —
авторизация не проходит. Заходить только через личный кабинет:

    reg.ru → Личный кабинет → Хостинги → «Хостинг Host-0 u3602414»
      → иконка панели управления в столбце «Действия»

Оттуда происходит автоматический вход в ISPmanager.
ISPmanager — это не reg.ru, а панель управления хостингом (продукт ISPsystem),
которую reg.ru ставит клиентам.

**Доступы к FTP/SSH** лежат в карточке услуги, вкладка «Доступы».
Там же кнопка сброса пароля. Логин: `u3602414`.
Пароль от FTP отдельный от пароля личного кабинета reg.ru.

## Выкладка

Архив из `dist/` → ISPmanager → Менеджер файлов → каталог сайта →
Загрузить → Извлечь. После распаковки выставить права:

    find . -type d -exec chmod 755 {} \; && find . -type f -exec chmod 644 {} \;

Иначе Apache отдаёт 403 — распаковщик ставит 600/700.

## Дизайн-токены

Все значения — цвета, отступы, тени, кегли — вынесены в `assets/tokens.css`.
Пока каждая страница держит свою копию токенов внутри `<style>`; при следующей
правке вёрстки их нужно заменить на подключение файла:

    <link rel="stylesheet" href="assets/tokens.css">

`assets/tokens.json` — те же токены в формате W3C Design Tokens. Его понимают
Figma Tokens и Style Dictionary, так что макет и код можно держать на одних
значениях.

Два слоя цвета: сырая палитра `--c-*` и семантика `--paper`, `--ink`, `--accent`.
В вёрстке использовать только вторую — тогда смена палитры не потребует правок
по всем файлам.

## Знать про вёрстку

- Дизайн-система IDS (из финансера): жёсткие тени без блюра, нулевые радиусы.
- Токены — в `:root`, шкалы отступов `--gap-*`, тени `--sh*`.
- Заголовок ограничен `min(3.2em, 7.6vw)`, чтобы не вылезал на узких экранах.
- Зерно бумаги — два слоя `body::before/::after`, крутится через `opacity`.
